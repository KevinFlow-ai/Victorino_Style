...
class NotificacionesRepositorioImpl implements NotificacionesRepositorio {
  ...
  Future<List<Notificacion>> obtenerBandeja() async {
    try {
      final resp = await _dio.get<List<dynamic>>(
        ...
        // Esto lo que hace es que el JWT del usuario se envíe automáticamente via JwtInterceptor.
        // Y el backend extraería el idUsuario del propio token y no haría falta query param.
      );
  ...
  Future<void> registrarDeviceToken(int idUsuario, String tokenFcm, {bool esLoginExplicito = false}) async {
    try {
      // El endpoint /notificaciones/fcm-token es público; por lo que necesita idUsuario en el body.
