...
// ============================================================
// ARCHIVO TEMPORAL DE PRUEBAS — ELIMINAR ANTES DE PRODUCCIÓN
// ============================================================
// Permite disparar cualquier tipo de notificación a cualquier
// usuario sin pasar por los flujos normales del negocio.
// Útil para verificar que push + in-app funcionan correctamente.
//
...
public class TestNotificacionesController {
    ...
    public ResponseEntity<Map<String, Object>> disparar(@RequestBody Map<String, Object> body) {
        // Extrae parámetros del body con valores por defecto razonables.
        ...
        String titulo = body.getOrDefault("titulo", "Notificación de prueba").toString();
        String cuerpo = body.getOrDefault("cuerpo", "Esta es una notificación de prueba desde el endpoint de test.").toString();
        ...
        try {
            ...
            Map<String, Object> err = new LinkedHashMap<>();
            err.put("ok", false);
            err.put("error", "Tipo de notificación inválido: " + tipo + ". Usa uno de: CONFIRMACION_RESERVA, RECORDATORIO_24H, CANCELACION_CLIENTE, CANCELACION_PELUQUERIA, NUEVA_CITA_EMPLEADO, CONTRASENA_ACTUALIZADA, AVISO_GENERAL");
            return ResponseEntity.badRequest().body(err);
        }

        // Dispara la notificación (in-app + push si el usuario lo permite).
        ...
        resp.put("mensaje", pushEnviada
                ...
                : "Notificacion in-app guardada. Push NO enviada (el destinatario no tiene tokens FCM registrados, tiene las notificaciones push desactivadas, o Firebase rechazó los tokens).");
